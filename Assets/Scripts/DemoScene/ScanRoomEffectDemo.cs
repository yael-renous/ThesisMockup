...
public class ScanRoomEffectDemo : RoomEffect
{
  ...
  void Start()
    ...
    void Update()
    {
        if (keyboard.hKey.wasPressedThisFrame)
        {
           ...
           SceneManager.Instance.debugAudioClip = audioClips[0];
        }
        if (keyboard.lKey.wasPressedThisFrame)
        {
            ActivateRoomScan(mediumPitchMaterial, originTransforms[0]);
            SceneManager.Instance.debugAudioClip = audioClips[1];
        }
        if(keyboard.mKey.wasPressedThisFrame){
            ActivateRoomScan(lowPitchMaterial, originTransforms[0]);
            SceneManager.Instance.debugAudioClip = audioClips[2];
        }
        ...
        if(playDebug&&keyboard.enterKey.wasPressedThisFrame){
            ...
            SceneManager.Instance.debugAudioClip = audioClips[randomMaterialIndex];
        }
    }
